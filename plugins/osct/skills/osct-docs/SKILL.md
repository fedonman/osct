...
Resolve `<osct-docs-skill>` to the directory containing this `SKILL.md`, then run:

```bash
uv run python "<osct-docs-skill>/scripts/check_docs.py" docs
```

Narrow it to the files you touched, which is the normal case:

```bash
uv run python "<osct-docs-skill>/scripts/check_docs.py" docs/guide/waveforms.md
```
